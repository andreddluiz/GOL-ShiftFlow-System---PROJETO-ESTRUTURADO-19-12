...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      console.log('[App] Usuário carregado:', user.email, 'Perfil:', user.perfil);
      setUsuario(user);
    }
    setAuthChecked(true);
  }, []);
  ...
  // Lógica de seleção de base inicial
  useEffect(() => {
    if (initialized && usuario && !selectedBase && bases.length > 0) {
      ...
      console.log('[App] Bases disponíveis para o usuário:', basesDisponiveis.length);
  ...
  // Filtrar as bases que este usuário pode selecionar
  const basesUsuario = dataAccessControlService.obterBasesAcessiveis(usuario, bases);
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden transform transition-all border dark:border-slate-700">
              ...
              <div className="p-8 text-center">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {basesUsuario.length > 0 ? (
                    basesUsuario.map(base => (
                      <button 
                        key={base.id} 
                    ...
                    <div className="col-span-full py-10">
                      <p className="text-red-500 font-bold">Nenhuma base encontrada ou vinculada ao seu perfil.</p>
                      <p className="text-sm text-gray-400">Contate o administrador do sistema.</p>
                    </div>
                  )}
                </div>
              </div>
