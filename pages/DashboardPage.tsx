...
import { 
  ...
  BarChart3, ArrowRightLeft, Filter, Globe, MapPin, ChevronRight, ListChecks,
  Search, Layout, History
} from 'lucide-react';
import { 
  Box, Typography, TextField, Grid, 
  Button, Tabs, Tab, ToggleButton, ToggleButtonGroup,
  Card, CardContent, Divider, Alert, FormControl, InputLabel, Select, MenuItem,
  Autocomplete, OutlinedInput, Chip
} from '@mui/material';
...
const DashboardPage: React.FC<{baseId?: string}> = ({ baseId }) => {
  ...
  const [dataFilter, setDataFilter] = useState<'todos' | 'diario' | 'mensal'>('todos');
  ...
  const [baseSelecionadaFiltro, setBaseSelecionadaFiltro] = useState<string>(baseId || 'all');
  ...
  const loadConsolidatedData = () => {
    ...
    const baseParaFiltrar = isAdmin ? baseSelecionadaFiltro : baseId;
  ...
  }, [baseSelecionadaFiltro, baseId, initialized, mesInicio, mesFim, bases, dataFilter]);
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
             ...
             <p className="text-xs font-black text-orange-600 uppercase tracking-widest">
               {baseSelecionadaFiltro === 'all' ? 'Rede GOL (Consolidado)' : `Base: ${bases.find(b => b.id === baseSelecionadaFiltro)?.sigla}`}
             </p>
          </div>
        </div>
        
        <Box sx={{ display: 'flex', gap: 3, alignItems: 'center' }}>
          {CURRENT_USER.permissao === PermissionLevel.ADMIN && (
            <FormControl size="small" sx={{ minWidth: 200 }}>
              <InputLabel children="Filtrar Base (Admin)" sx={{ fontWeight: 800, fontSize: '0.7rem' }} />
              <Select
                value={baseSelecionadaFiltro}
                label="Filtrar Base (Admin)"
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={categoryChartData} layout="vertical" margin={{ top: 10, right: 100, left: 100, bottom: 10 }}>
                      ...
                      <Bar dataKey="mensal" name="Mensal" stackId="a" fill="#0ea5e9" radius={[0, 4, 4, 0]}>
                        {/* Solicitação: Exibir apenas o total consolidado à direita para evitar sobreposição */}
